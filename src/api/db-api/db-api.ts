...
export const updateByFilename = async (text: string, duration: number, file_name: string, model: string,transcribe_times:number) => {
    return wrapper(async (db) => {
        return await db.execute(
            "UPDATE transcribed_files SET text=$1, duration=$2,model=$3,update_timestamp=$4,transcribe_times=$5 WHERE file_name=$6",
            [text, duration, model, Date.now(),transcribe_times,file_name,]
        );
    })
}
